...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	if($page==1)
	{
		...
		$prepage='<a class="pre none" target=\"_self\"><em>'.$fun_r['pripage'].'</em></a>';
	}
	...
	if($page<>1)
	{
		...
		$prepage='<a href="'.$dolink.$prido.'" class="pre" target=\"_self\"><em>'.$fun_r['pripage'].'</em></a>';
	}
	//下一页
	
	if($page==$totalpage)
	{
		$nextpage='<a class="next none" target=\"_self\"><em>'.$fun_r['nextpage'].'</em></a>';
		$lastpage='<a class="zi none" target=\"_self\"><em>'.$fun_r['lastpage'].'</em></a>';
	}
	...
	if($page!=$totalpage)
	{
		...
		$nextpage='<a href="'.$nextpagelink.'" class="next" target=\"_self\"><em>'.$fun_r['nextpage'].'</em></a>';
...
function sys_ShowTextPage($totalpage,$page,$dolink,$add,$type,$search=""){
	...
	if($page<>1)
	{
		$toppage='<a href="'.$dolink.$add[filename].$type.'" target=\"_self\">'.$fun_r['startpage'].'</a>';
		$pagepr=$page-1;
		if($pagepr==1)
		{
	...
	{
		$pagenex=$page+1;
		$nextpage='<a href="'.$dolink.$add[filename].'_'.$pagenex.$type.'" class="next" target=\"_self\"><em>'.$fun_r['nextpage'].'</em></a>';
		$lastpage='<a href="'.$dolink.$add[filename].'_'.$totalpage.$type.'" target=\"_self\">'.$fun_r['lastpage'].'</a>';
	}
	$starti=$page-$snum<1?1:$page-$snum;
	$no=0;
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		elseif($i==1)
		{
			$is_1='<a href="'.$dolink.$add[filename].$type.'" target=\"_self\">';
			$is_2="</a>";
		}
		else
		{
			$is_1='<a href="'.$dolink.$add[filename].'_'.$i.$type.'" target=\"_self\">';
			$is_2="</a>";
		}
		$returnstr.=$is_1.$i.$is_2;
	}
